...
void DFS(int x,int y){
    ...
    for(int i=0;i<4;i++){
        ...
        if(out(qx,qy)) continue;    //맵을 벗어나면 SKIP
        if(vec[qx][qy] !=0) continue;   //0이 아니라면 SKIP
        vec[qx][qy] = 2;
        DFS(qx,qy);
    }
    return;
...
int search(){
    for(auto cord : virus){
        DFS(cord.first,cord.second);    //바이러스 확산
    }
    int cnt =0;
    for(int i=0;i<N;i++){
        for(int j=0;j<M;j++){
    ...
    for(int i=0;i<N;i++){
        for(int j=0;j<M;j++){
            if(vec[i][j] == 2) vec[i][j] = 0;   //원복
        }
    }
    for(auto cord : virus){
        vec[cord.first][cord.second] = 2;   //다음 DFS를 위해 바이러스 원복
    }
    return cnt;
}
...
int main(){
    ...
    for(int i=0;i<wall.size();i++){
        for(int j=i+1;j<wall.size();j++){
            for(int k = j+1;k<wall.size();k++){     //벽을 세울 3개의 조합 뽑아낸다.
